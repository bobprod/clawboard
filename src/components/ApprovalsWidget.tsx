...
import { ShieldCheck, ShieldX, Clock, User, Bot, RefreshCw, CheckCheck, XCircle, Info } from 'lucide-react';
...
export const ApprovalsWidget = () => {
  ...
  const fetchApprovals = useCallback(() => {
    ...
    apiFetch(`${BASE}/api/approvals`)
      .then(r => r.json())
      .then((data: ApprovalRequest[]) => { setRequests(data); setLoading(false); })
      .catch(() => {
        // Fallback mock quand l'endpoint n'existe pas encore
        setRequests([
          {
            id: 'apr-1',
            taskId: 'task-42',
